...
$idenf=$_SESSION['id'];
$sql = "SELECT logado FROM enfermeiro WHERE enfermeiroid=$idenf";
...
$sql = "SELECT primeironome FROM enfermeiro
		INNER JOIN funcionario ON funcionario.funcid=enfermeiro.funcid
		INNER JOIN pessoa ON pessoa.pessoaid=funcionario.pessoaid WHERE enfermeiro.enfermeiroid=$idenf";
$stmt = $con->prepare($sql);
$stmt->execute();
$linha=$stmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
	...
	<script type="text/javascript" src="enfermeiroMain.js"></script>
</head>
<body>
	<nav>
		<ul id="ul-principal">
			<?php //echo"<p class='cardValor fl-right' style='font-size:30px; color:#e53935'> $date_last </p>"?>
			<?php echo"<p class='login_d'> $temp<br>enfermeiro </p>";?>
			<li class="li-p"><a href="">HOME</a></li>
			<li class="li-p">
				<button id="btn_paci" class="btn_menu">visualizar pacientes</button>
			</li>
			<li class="li-p">
				<button id="btn_medi" class="btn_menuM">visualizar medicamentos</button>
			</li>
			<li class="li-p">
				<button id="btn_Rmedi" class="btn_menu">retirar medicamento</button>
			</li>
	...
	<!--retirar medicamento-->
	<div id="medicamento_retirar" class="opt_screen inputt">
		<?php
			$sql = "SELECT * FROM medicamento";
			$stmt = $con->prepare($sql);
			$stmt->execute();
		...
		<form method="POST" action="retirar_medi.php">
			<div class="container">
				<label for="">medicamento a ser retirado</label>
				<div class="selectBox">
				...
				<input type="submit" class="btn_menu" name="btn_retirar" value="retirar">
			</div>
		</form>
	</div>
